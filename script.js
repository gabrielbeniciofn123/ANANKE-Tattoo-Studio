(() => {
  ...
  $('#artists-grid').hidden = content.artists.length === 0;
  content.artists.forEach((artist, index) => { const card = element('article', 'artist-card'); const photo = element('div', 'artist-photo'); photo.append(element('span', 'artist-number', `0${index + 1}`)); if (artist.photo) { const img = element('img'); img.src = safeURL(artist.photo); img.alt = artist.name; img.loading = 'lazy'; photo.append(img); } else { photo.append(element('span', 'artist-monogram', ['a.', 'n.', 'k.'][index % 3]), element('span', 'artist-placeholder-label', 'EM BREVE')); } const info = element('div', 'artist-info'); const text = element('div'); text.append(element('h3', '', artist.name), element('p', '', artist.specialty)); info.append(text); if (artist.instagram && safeURL(artist.instagram)) { const link = element('a', '', '↗'); link.href = safeURL(artist.instagram); link.target = '_blank'; link.rel = 'noopener noreferrer'; link.setAttribute('aria-label', `Instagram de ${artist.name}`); info.append(link); } card.append(photo, info); $('#artists-grid').append(card); });
  if (content.artists.length && content.artists.every(artist => artist.photo)) $('.artists-footer p').textContent = 'Encontre o olhar que combina com a sua próxima história.';
